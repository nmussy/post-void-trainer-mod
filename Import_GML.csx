...
// Edited by Samuel Roy

// Found on the Underminers Discord FAQ
// https://discord.com/invite/RxXpdwJ
// https://discordapp.com/channels/566861759210586112/617135933782884368/696063953620566047
...
foreach (string file in dirFiles) {
	...
	UpdateProgressBar(null, "Files (" + fileName + ")", progress++, dirFiles.Length);
	...
	if (!(Data.IsGameMaker2()) && (fileName.Contains("_PreCreate_0.gml") || fileName.Contains("Asynchronous") || fileName.Contains("Unknown13")))
	{
		continue; // Restarts loop if trying to import GMS2 code into GMS1.
	}
	if (!fileName.EndsWith(".gml") || !fileName.Contains("_")) // Perhaps drop the underscore check?
		continue; // Restarts loop if file is not a valid code asset.
